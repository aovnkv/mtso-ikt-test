...
const LoginForm = ({ authenticateUser, isSubmitting }) => {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    try {
      const res = await authenticateUser(formValues); //валидные параметры доступа max@test.com / 12345
  ...
  const warning = formValues.showMessage && (
  ...
  if (userID)
    return (
      <div className="content">
        <h1>Вы уже вошли на сайт как {userID}</h1>
      </div>
    );
